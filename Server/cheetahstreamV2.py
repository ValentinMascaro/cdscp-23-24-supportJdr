...
url='http://localhost:1880/commande/'

def send_to_node_red(transcription):
    # Modify the URL to match your Node-RED endpoint
    base_node_red_url = 'http://localhost:1880/commande/'
    ...
    try:
        for word in words:
            ...
            word_node_red_url = base_node_red_url + word
            print('Sending word to Node-RED: {}'.format(word))
            
            # Send the word to Node-RED using GET request
            response = requests.get(word_node_red_url)

            if response.status_code == 200:
                print('Successfully sent word to Node-RED: {}'.format(word))
            else:
                print('Failed to send word to Node-RED: {}'.format(word))
    except Exception as e:
        print('Error sending transcription to Node-RED:', e)
    ...
    try:
        ...
        try:
            while True:
                ...
                if is_endpoint:
                    full_transcript = cheetah.flush()
                    send_to_node_red(full_transcript)
